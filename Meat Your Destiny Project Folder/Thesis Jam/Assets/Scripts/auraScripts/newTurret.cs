...
public class TurretScript : MonoBehaviour
{
        ...
        if (col.gameObject.GetComponent<AuraGenerator>())
        {
            if (contestable)
            {
                ...
                {//set the owner to whoever hits the turret when the turret is unowned
                    ownerNum = col.gameObject.GetComponent<AuraGenerator>().auraPlayerNum;
                    litSegments = 1;
    ...
    void CreateNewTurret()
    {
        ...
        TurretScript newTurret = Instantiate(turretTypes[timesOwned - 1], transform.position, Quaternion.identity).GetComponent<TurretScript>();
