Shader "SineVFX/LivingParticles/LivingParticleMaskedPbrArray"
{
	...
	SubShader
	{
		...
		void vertexDataFunc( inout appdata_full v, out Input o )
		{
			...
			if(w == 0){
			DistanceMask45 = distance( appendResult17 , _Affectors[w] );
			}else{
			DistanceMask45 = min( DistanceMask45, distance( appendResult17 , _Affectors[w] ) );	
			}
			}
			//float DistanceMask45 = distance( appendResult17 , _Affector );
